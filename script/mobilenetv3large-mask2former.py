"""
Hybrid MobileNetV3-Large + Mask2Former training script for BigPlants-100.

Main idea:
1) Use Mask2Former (Swin-L) to estimate foreground regions.
2) Apply foreground mask to the input image.
3) Feed masked/focused image into MobileNetV3-Large for classification.

This script keeps the same dataset curation and pHash leakage-handling logic style
as the standalone classification script, while adding the hybrid segmentation-guided
classification branch and qualitative visualization outputs.
"""
...
# Optional import for Mask2Former
try:
...
# ============================================================
# 2. Dataset Curation (same strategy as standalone)
# ============================================================
...
# ============================================================
# 3. pHash leakage handling (ported from standalone)
# ============================================================
...
def get_transforms(img_size=224):
	...
	Classification normalization is done inside hybrid model.
	"""
...
# ============================================================
# 5. Hybrid Model: Mask2Former foreground + MobileNetV3 classifier
# ============================================================
...
class HybridMobileNetMask2Former(nn.Module):
	"""
	Hybrid architecture:
	  - Segmentation branch: frozen Mask2Former (Swin-L)
	  - Classification branch: MobileNetV3-Large
	  - Fusion: mask-guided input focusing (image * focus_map)
	"""
	...
	def __init__(
		...
		# ImageNet normalization for MobileNet branch
	...
	def _predict_foreground_mask(self, images_raw: torch.Tensor) -> torch.Tensor:
		"""
		Input: images_raw in [0,1], shape (B,3,H,W).
		Output: foreground mask in [0,1], shape (B,1,H,W).
		"""
		...
		# semantic_maps: list of (H, W), class ids
...
def save_qualitative_examples(
	...
	"""Save visualization panels: original, mask, focused image, and labels."""
...
def main():
	parser = argparse.ArgumentParser(description="Hybrid MobileNetV3-Large + Mask2Former for BigPlants-100")

	# Data
	parser.add_argument("--data_root", type=str, required=True)
	parser.add_argument("--out_dir", type=str, default="./outputs_mobilenetv3large-mask2former")
	...
	model = HybridMobileNetMask2Former(
