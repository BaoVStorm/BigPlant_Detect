"""
Hybrid MobileNetV3-Large + DeepLabV3 training script for BigPlants-100.

Main idea:
1) DeepLabV3-ResNet50 produces a foreground mask from input image.
2) The foreground mask is applied to input image (attention-style masking).
3) Masked image is fed into MobileNetV3-Large for 100-class classification.
...
- Mixed precision, DataParallel, WeightedRandomSampler
- Metrics, confusion matrix, reports, checkpoints, training history
- Visualization for paper: original / mask / masked / prediction vs ground truth
"""
...
# ============================================================
# Hybrid model (DeepLabV3 mask + MobileNetV3 classifier)
# ============================================================

class HybridMobileNetV3DeepLabV3(nn.Module):
	"""
	Hybrid architecture:
	- Segmentation branch: DeepLabV3-ResNet50
	- Classification branch: MobileNetV3-Large

	Forward flow:
	input image -> segmentation logits -> foreground mask -> masked image -> classifier logits
	"""
	...
	def __init__(
		...
		# Build MobileNetV3-Large branch
		...
		try:
			...
			tv_mobilenet = tv_models.mobilenet_v3_large(weights=weights)
			in_features = tv_mobilenet.classifier[3].in_features
			tv_mobilenet.classifier[3] = nn.Linear(in_features, num_classes)
			self.cls_model = tv_mobilenet

	def _build_foreground_mask(self, x: torch.Tensor, seg_logits: torch.Tensor) -> torch.Tensor:
		"""
		Build a foreground mask from segmentation logits.
		For torchvision pretrained DeepLabV3 (VOC-like labels), class-0 is background.
		We use foreground probability = 1 - P(background).
		"""
...
def save_visualization_samples(
	...
	1) Original image
	2) Segmentation foreground mask
	3) Masked image used by classifier
	4) Prediction vs Ground Truth text panel
	"""
...
def main():
	...
	parser.add_argument("--out_dir", type=str, default="./outputs_mobilenetv3large-deeplabv3")
	...
	parser.add_argument("--mask_mode", type=str, default="attention", choices=["attention", "hard", "residual"])
	...
	print("\n[6/8] Building hybrid model (DeepLabV3 + MobileNetV3-Large)...")
	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
	model = HybridMobileNetV3DeepLabV3(
