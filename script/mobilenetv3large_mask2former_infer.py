...
try:
    ...
    raise ImportError("transformers is required for mobilenetv3large-mask2former inference") from exc
...
class MobileNetV3Mask2FormerClassifier(nn.Module):
    def __init__(
        self,
        num_classes: int,
...
class MobileNetMask2FormerInferenceAdapter(nn.Module):
    def __init__(self, model: MobileNetV3Mask2FormerClassifier):
        super().__init__()
        self.model = model
