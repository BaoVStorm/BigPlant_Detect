# python resnet50-segformer.py --data_root /path/to/bigplants-100 --out_dir ./outputs_resnet50-segformer --epochs 30 --batch_size 32 --lr 1e-4 --num_workers 8
"""
Hybrid ResNet50 + SegFormer-B4 for BigPlants-100 classification.

Concept:
- SegFormer generates semantic foreground masks (plant-related regions)
- Images are masked to reduce background noise
- ResNet50 classifies 100 plant species from the focused image
...
# =============================================================================
# Hybrid model: SegFormer + ResNet50
# =============================================================================

def build_resnet50(num_classes: int):
	"""
	Build a ResNet50 classification backbone.
	Attempts timm first (higher quality weights), falls back to torchvision.
	"""
	try:
		import timm
...
class HybridResNet50SegFormer(nn.Module):
	"""
	Hybrid module combining SegFormer segmentation with ResNet50 classification.

	Architecture overview:
	  1) SegFormer predicts per-pixel semantic logits from the input image.
	  2) A foreground probability mask is derived from the SegFormer output,
	     highlighting plant-related regions (leaves, flowers, fruits, etc.).
	  3) The mask is applied to the original image, creating a "focused image"
	     that retains foreground details while suppressing background noise.
	  4) ResNet50 classifies the focused image into one of 100 plant species.

	The SegFormer branch is frozen by default to act as a fixed feature
	extractor, while only the ResNet50 classification head is trained.
	This prevents the large SegFormer model from dominating the gradient
	updates and makes training more memory-efficient.

	Parameters:
	  - mask_blend:     Controls the overall masking strength (0 = no mask, 1 = full).
	  - background_keep: Fraction of background intensity retained (prevents complete
	                     blackout of non-plant regions, which could hurt classification
	                     when the segmenter is imprecise).
	"""
	def __init__(self,
				 num_classes: int,
		...
		super().__init__()
		# --- Segmentation branch: SegFormer-B4 ---
		...
		# Determine which ADE20K classes correspond to plant material
		...
		# Freeze SegFormer so only ResNet50 is trained
	...
	def _build_foreground_mask(self, seg_logits: torch.Tensor, out_h: int, out_w: int) -> torch.Tensor:
		"""
		Convert raw SegFormer logits into a soft foreground probability mask.

		Steps:
		  1) Up-sample logits to match the input image resolution.
		  2) Apply softmax to get per-class probabilities.
		  3) Aggregate probabilities of plant-related classes to form the
		     foreground mask. If no plant classes were identified, fall back
		     to using the maximum class probability as a saliency proxy.
		...
		if len(self.plant_class_ids) > 0:
			...
			if len(ids) > 0:
				# Sum probabilities of all plant-related semantic classes
				fg_prob = probs[:, ids, :, :].sum(dim=1, keepdim=True)
			else:
				fg_prob = probs.max(dim=1, keepdim=True).values
		else:
			# Fallback: use max prob as a crude saliency map
			fg_prob = probs.max(dim=1, keepdim=True).values
	...
	def forward(self, x: torch.Tensor, return_aux: bool = False):
		...
		Args:
		  x:          Input image tensor (B, 3, H, W), normalized.
		  return_aux: If True, also return the foreground mask and focused image
		              (used for visualization and paper figures).

		Returns:
		  logits:  Classification logits from ResNet50 (B, num_classes).
		  (optional) fg_mask:  Foreground probability mask (B, 1, H, W).
		  (optional) focused:  The masked input image (B, 3, H, W).
		"""
		# --- Step 1: SegFormer segmentation (frozen or trainable) ---
		if self.freeze_segformer:
			with torch.no_grad():
		...
		# --- Step 2: Build foreground attention mask ---
		fg_mask = self._build_foreground_mask(seg_out.logits, x.shape[-2], x.shape[-1])

		# --- Step 3: Apply mask to create focused image ---
		# effective_mask blends between full pass-through (no masking) and
		# foreground-only, while always retaining `background_keep` fraction
		# of the background to avoid harsh black-outs.
		...
		# --- Step 4: ResNet50 classification on the focused image ---
...
def save_hybrid_visualizations(model, dataset, species_list, device, out_dir: Path,
							   mean, std, n_samples=40, seed=42):
	"""
	Save qualitative figures demonstrating the hybrid model's behavior.

	For each sampled test image, a 4-panel subplot is saved:
	  1) Original Image       - the raw input after inverse normalization.
	  2) Foreground Mask      - the SegFormer-predicted probability mask
	                            showing which pixels the model considers
	                            plant-related (brighter = higher confidence).
	  3) Focused Image        - the input after mask application, visualizing
	                            how the background is suppressed.
	  4) Labels Panel         - Ground Truth vs Predicted species name, with a
	                            color indicator (green = correct, red = wrong).

	These figures serve as evidence in the paper that SegFormer successfully
	guides ResNet50 to attend to the relevant plant structures (leaves,
	flowers, fruits) rather than noisy backgrounds.
	"""
	...
	print(f"[VIS] Done. Saved {n} figures to {vis_dir}")
...
def main():
	parser = argparse.ArgumentParser(description="Hybrid ResNet50 + SegFormer-B4 on BigPlants-100")
	parser.add_argument("--data_root", type=str, required=True)
	parser.add_argument("--out_dir", type=str, default="./outputs_resnet50-segformer")
	...
	parser.add_argument("--img_size", type=int, default=224,
						help="Input image size: 224, 384, or 512")
	...
	parser.add_argument("--vis_samples", type=int, default=40, help="Number of test visualization samples")
	...
	print(f"[CONFIG] Hybrid ResNet50 + SegFormer-B4 | img_size={args.img_size}")
	print("=" * 90)
	...
	# 6) Build model
	# =========================================================================
	print("\n[6/8] Building hybrid model (ResNet50 + SegFormer-B4)...")
	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

	model = HybridResNet50SegFormer(
	...
	print(f"ResNet50 backend: {model.module.cls_backend if isinstance(model, nn.DataParallel) else model.cls_backend}")
